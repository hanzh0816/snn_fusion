...
from core.model.module.snn_module import DFL, SpikeConv


@MODEL_REGISTRY.register()
class SpikeYOLOHead(nn.Module):
    @configurable
    def __init__(
        self, ch, num_classes, stride, weight, spike_cfg=None, nms_cfg=None, *args, **kwargs
    ):
        super().__init__()
        self.in_channels = ch
    ...
    def forward(self, x: List[Tensor]):
        ...
        for i in range(self.num_layers):
            x[i] = torch.cat((self.reg_conv[i](x[i]), self.cls_conv[i](x[i])), dim=2)
            x[i] = x[i].mean(0)

        # fire_rate = np.mean([x[i].detach().mean().cpu().numpy()])
        # print(f"fire_rate:{fire_rate}")
        return x
