...
class TaskAlignedAssigner(nn.Module):
    ...
    def forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        self.n_max_boxes = gt_bboxes.size(1)  # boxes存在的目标个数
        ...
        mask_pos0sum = mask_pos[0].sum(0)
        # print("没有一个grid预测多个目标的情况！")
        # if mask_pos0sum.max() >1:
        #     print("存在一个grid预测多个目标的情况！")
        # mask_pos shape:[B,boxes,8400] 本质是一个01矩阵，表征了每个格子负责预测那个对象。每个对象都被10(超参数)个格子预测。所以sum的结果是10
        # 但这样存在一个问题，如果如果一个格子中有两个物体，那么这两个物体都由这一个格子来预测。又由于偏移是正向的，因此无法通过别的格子来预测
        # 可能可以通过向偏移增加一个负数卷积来解决，或者增加预测的个数  overlaps参数估计返回了每个预测和真实结果的阈值
        # 事实上，一个格子要预测两个目标是很常见的，因为每个目标被多个格子预测，如果两个目标挨得很近，那么中间这个格子大概率会被用到两次。
        target_gt_idx, fg_mask, mask_pos = select_highest_overlaps(
            mask_pos, overlaps, self.n_max_boxes
        )
        # 对于一个格子要预测多个目标的情况，则这个格子会负责预测与他预测结果roi最高的目标
        # Assigned target
        target_labels, target_bboxes, target_scores = self.get_targets(
            gt_labels, gt_bboxes, target_gt_idx, fg_mask
        )
        # target_labels shape:[b,8400]  简单来说，上面这个函数的作用就是把每负责的十个格子和他们要预测的目标对应起来
        # Normalize
...
def dist2bbox(distance, anchor_points, xywh=True, dim=-1):
    ...
    # 将distance张量按照dim维度分成两部分，分别表示左上角(lt)和右下角(rb)的距离
