...
D2_LOG_BUFFER_SIZE_KEY: str = "D2_LOG_BUFFER_SIZE"  #  定义一个常量，用于表示日志缓冲区大小的配置键

DEFAULT_LOG_BUFFER_SIZE: int = (
    1024 * 1024
)  # 1MB #  定义默认的日志缓冲区大小为1MB（1024 * 1024字节）
...
def setup_logger(
    ...
    *,
    ...
    if comm.is_main_process() and configure_stdout:
        ch = logging.StreamHandler(stream=sys.stdout)
        # tag: stdout level is INFO
